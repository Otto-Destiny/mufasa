"""Converted from kaggle-extraction.ipynb.
...
# # MUFASA extraction on Kaggle
#
# Three plain calls per paper - observations, study context and profile, training
# examples - written straight to Parquet.
#
# Set the knobs in the first cell, run the rest in order. The bar advances once
# per task, so three times per paper. Finished papers are skipped on a re-run, so
# stopping and restarting is safe.
#
# **Before running:** mount the corpus dataset (it must contain
# `mufasa_corpus/parsed/markdown`), and add your API key as a Kaggle Secret named
# whatever `KEY_NAME` says.
...
# The folder that CONTAINS mufasa_corpus. Set it outright if you know where the
# dataset landed; leave it None to search /kaggle/input and the repo layout.
CORPUS_PATH = "/kaggle/input/datasets/destinyotto/mufasa-llm-extraction"
...
def find_corpus():
    if CORPUS_PATH:
        folder = Path(CORPUS_PATH)
        if holds_corpus(folder):
            return folder
        print(f"CORPUS_PATH is set but {folder}/mufasa_corpus/parsed/markdown "
              "is not there - searching instead")
    candidates = []
    mounted = Path("/kaggle/input")
    if mounted.is_dir():
        candidates.append(mounted)
        # a dataset can sit a few folders down, so walk a little
        level = [mounted]
        for _ in range(3):
    ...
    for folder in (here, *here.parents):
        candidates.append(folder / "01-data-engineering" / "data-extraction")
...
if CORPUS is None:
    raise SystemExit(
        "mufasa_corpus/parsed/markdown was not found. Set CORPUS_PATH above to "
        "the folder that contains mufasa_corpus.")

# Where results are written. /kaggle/working persists with a saved version.
OUT_ROOT = Path("/kaggle/working") if Path("/kaggle/working").is_dir() else CORPUS

BATCH_START = 1          # inclusive
BATCH_END = 10           # inclusive
BATCHES = 10             # how many batches the papers are split into
PAPERS = 100             # total papers, taken from the top of the corpus

MODEL = "gpt-5.6-sol"
BASE_URL = "https://cavoti.com/v1"
REASONING_EFFORT = ""            # "" for none, or minimal|low|medium|high|xhigh
EXTRA_BODY = None                # a gateway needing its own object sets this
...
WORKERS = 1              # papers at once
WORKERS_PER_KEY = 1      # requests in flight on any single key
WRITE_EVERY = 5          # rebuild the Parquet tables every N finished papers
ATTEMPTS = 6             # per request; it also drops slow-starting ones
MAX_TOKENS = 120000
...
print(f"batches {BATCH_START}-{BATCH_END} of {BATCHES}, {PAPERS} papers, "
      f"{WORKERS} at a time, model {MODEL}")
