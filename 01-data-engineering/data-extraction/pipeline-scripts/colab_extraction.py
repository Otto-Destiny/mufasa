"""Converted from colab-extraction.ipynb.
...
# # MUFASA extraction on Colab
#
# Three plain calls per paper - observations, study context and profile, training
# examples - written straight to Parquet.
#
# Run the cells in order. The bar advances once per task, so three times per
# paper. Finished papers are skipped on a re-run, so a disconnect costs only the
# paper that was in flight.
#
# **Before running:** paste your keys into `API_KEYS` in the first cell, and put
# `mufasa-llm-claim-extraction-corpus.zip` at the top of your Google Drive. Output
# is written to `MyDrive/mufasa/extraction_output`, which survives a disconnect.


# =============================== configuration ===============================
# Everything you would normally change lives here.
import zipfile
from pathlib import Path

# Google Drive holds the corpus and keeps the results: /content is wiped when
# the runtime disconnects, and this run takes hours.
MOUNT_DRIVE = True
DRIVE = Path("/content/drive/MyDrive")

if MOUNT_DRIVE and not DRIVE.is_dir():
    from google.colab import drive as _drive
    _drive.mount("/content/drive")

# The folder that CONTAINS mufasa_corpus. Leave None to search Drive and
# /content, or set it outright once you know where it landed.
CORPUS_PATH = None

# If the corpus is not found, this zip is extracted to /content and used.
# Upload mufasa-llm-claim-extraction-corpus.zip to the top of your Drive.
ZIP_PATH = "/content/drive/MyDrive/mufasa-llm-claim-extraction-corpus.zip"

# Paste your keys here, one per line. Add as many as you have; requests are
# handed out across all of them in turn, so the list length is the only thing
# that changes when you add another.
API_KEYS = [
    "PASTE_YOUR_KEY_HERE",
]
API_KEYS = [key.strip() for key in API_KEYS
            if key.strip() and not key.startswith("PASTE_")]

# ---- which papers ----------------------------------------------------------
# BATCHES divides PAPERS, so the batch size is PAPERS / BATCHES. Batch numbers
# are absolute positions in the pool, not counted from where you start - batch 4
# is always papers 301-400 when the batches are 100 long.
#
# For batches of 100, keep BATCHES = PAPERS / 100. Every row here was checked
# against the arithmetic below and lands on exactly the papers it names:
#
#     papers to run   PAPERS  BATCHES  BATCH_START  BATCH_END   batches
#        301-1000       1000       10            4         10         7   <-- now
#       1001-2000       2000       20           11         20        10
#       2001-3000       3000       30           21         30        10
#       3001-4000       4000       40           31         40        10
#       4001-5000       5000       50           41         50        10
#       5001-6000       6000       60           51         60        10
#       6001-7000       7000       70           61         70        10
#       7001-8000       8000       80           71         80        10
#       8001-9000       9000       90           81         90        10
#       9001-10000     10000      100           91        100        10
#      10001-10321     10321      104          101        104         4   <-- the rest
#
# The pattern: BATCHES = PAPERS / 100, and BATCH_START = first paper / 100 + 1.
#
# Only the last row departs from it, because 10,321 does not divide by 100. The
# batch size is a ceiling division, so BATCHES has to round UP to 104: at 103 the
# batches come out 101 papers long and the numbering no longer lines up with the
# rows above. Its final batch holds the remaining 21 papers rather than 100.
#
# Papers already finished are skipped before any API call, so widening the range
# backwards costs nothing but a moment - and running the whole corpus in one go
# is PAPERS=10321, BATCHES=104, BATCH_START=1, BATCH_END=104.
BATCH_START = 4          # inclusive
BATCH_END = 10           # inclusive
BATCHES = 10             # 1000 / 10 = batches of 100
PAPERS = 1000            # total papers, taken from the top of the corpus

# ---- gateway ---------------------------------------------------------------
# Both gateways are kept here so switching is one word. The Cavoti settings
# took a long while to get right, so they stay exactly as they were - put
# "cavoti" back in PROVIDER and paste Cavoti keys into API_KEYS above.
PROVIDERS = {
    "cavoti": {
        ...
        # varies run to run - which is why the retries matter most here.
    },
    "tokenrouter-deepseek": {
        ...
        # at all with finish_reason "stop", which is retried as a failure.
    },
    "openrouter": {
        ...
        # minutes for one task. "low" is the lever, and with it the model
        # spends its whole budget on the answer instead - measured at zero
        # reasoning tokens and roughly twice the speed of anything else here.
...
PROVIDER = "openrouter"                  # or "cavoti", "tokenrouter-deepseek"
BASE_URL = PROVIDERS[PROVIDER]["base_url"]
MODEL = PROVIDERS[PROVIDER]["model"]
REASONING_EFFORT = PROVIDERS[PROVIDER]["reasoning_effort"]
EXTRA_BODY = PROVIDERS[PROVIDER].get("extra_body")   # None for the others

# ---- how hard to push ------------------------------------------------------
# Papers run in parallel and a paper's three tasks run one after another, so
# WORKERS is also the number of requests in flight. The real ceiling is
# min(WORKERS, keys x WORKERS_PER_KEY), so paste more keys to lift it.
WORKERS = 10             # papers at once
WORKERS_PER_KEY = 2      # requests in flight on any single key
WRITE_EVERY = 25         # rebuild the Parquet tables every N finished papers;
                         # each rebuild re-reads every raw file, so keep this
                         # well above 5 once the run is hundreds of papers long
ATTEMPTS = 6             # per request
MAX_TOKENS = 120000      # thinking is spent out of this, so keep it wide
OUT = "extraction_output"
REDO = False             # True re-extracts papers already finished


def holds_corpus(folder):
    return (folder / "mufasa_corpus" / "parsed" / "markdown").is_dir()


def find_corpus():
    if CORPUS_PATH and holds_corpus(Path(CORPUS_PATH)):
        return Path(CORPUS_PATH)
    roots = [Path("/content"), DRIVE, Path.cwd()]
    candidates = [root for root in roots if root.is_dir()]
    level = list(candidates)
    for _ in range(2):                       # a folder or two deep is plenty
        level = [child for folder in level if folder.is_dir()
                 for child in sorted(folder.iterdir()) if child.is_dir()]
        candidates += level
    for folder in candidates:
        if holds_corpus(folder):
            return folder
    return None


CORPUS = find_corpus()
if CORPUS is None and ZIP_PATH and Path(ZIP_PATH).is_file():
    print(f"extracting {ZIP_PATH} - this takes a couple of minutes")
    with zipfile.ZipFile(ZIP_PATH) as archive:
        archive.extractall("/content")
    CORPUS = find_corpus()
if CORPUS is None:
    raise SystemExit(
        "mufasa_corpus/parsed/markdown was not found. Upload the corpus zip to "
        "Drive and set ZIP_PATH, or set CORPUS_PATH to the folder holding "
        "mufasa_corpus.")

# Results go to Drive so a disconnect does not lose them.
OUT_ROOT = DRIVE / "mufasa" if DRIVE.is_dir() else Path("/content")
OUT_ROOT.mkdir(parents=True, exist_ok=True)

if not API_KEYS:
    raise SystemExit("paste at least one key into API_KEYS above")

print("corpus:", CORPUS)
print("output:", OUT_ROOT / OUT)
print(f"keys: {len(API_KEYS)}  ->  up to "
      f"{min(WORKERS, len(API_KEYS) * WORKERS_PER_KEY)} request(s) at once")
print(f"batches {BATCH_START}-{BATCH_END} of {BATCHES}, {PAPERS} papers, "
      f"model {MODEL}")
...
# pyarrow is what to_parquet writes through; it ships in the Colab and Kaggle
# images, but a missing one would only surface after hours of extraction.
for _module, _package in (("openai", "openai>=1.40,<3"), ("pyarrow", "pyarrow")):
...
# One client per key, each with a semaphore so no key carries more than
# WORKERS_PER_KEY requests at once. A shared counter hands out the next key, so
# two workers starting at the same moment do not land on the same one.
POOL = [(OpenAI(base_url=BASE_URL, api_key=key, max_retries=0, timeout=1800),
         threading.BoundedSemaphore(WORKERS_PER_KEY)) for key in API_KEYS]
...
def next_slot():
    with _TURN_LOCK:
        return next(_TURN) % len(POOL)


print(f"{len(POOL)} client(s) ready")

def eligible_papers(limit):
    """The corpus this run may touch: parsed, rights-cleared, not retracted."""
    documents = pd.read_parquet(CORPUS / "mufasa_corpus" / "manifests" / "documents.parquet")
...
def ask(clients, model, prompt, paper, max_tokens, attempts, reasoning_effort,
        task_name=""):
    ...
    for attempt in range(1, attempts + 1):
        # the next key in turn, held for the length of the request, so a retry
        # moves on rather than queueing behind the key that just failed
        client, limit = clients[next_slot()]
        try:
          with limit:
              extra = {"reasoning_effort": reasoning_effort} if reasoning_effort else {}
              # A gateway that wants its own object says so in PROVIDERS;
              # OpenRouter's reasoning switch arrives this way, and that
              # endpoint refuses to run without it.
              if EXTRA_BODY:
...
def extract_paper(clients, row, options, on_task=None, on_task_done=None):
    """Three tasks, one after the other, for a single paper."""
    paper_id = row["paper_id"]
    text = (CORPUS / "mufasa_corpus" / "parsed" / "markdown" / f"{paper_id}.md").read_text(
        encoding="utf-8")
    record = {
        "paper_id": paper_id,
    ...
    for name, prompt in TASKS:
        ...
        result = ask(clients, options.model, prompt, text, options.max_tokens,
...
def run(batch_start=1, batch_end=10, batches=10, papers=100,
        out="extraction_output", model="gpt-5.6-sol",
        base_url="https://cavoti.com/v1",
        workers=1, attempts=6, max_tokens=120000, reasoning_effort="",
    ...
    options = Options(
        ...
        papers=papers, out=out, model=model, base_url=base_url,
        workers=workers, attempts=attempts,
        max_tokens=max_tokens, reasoning_effort=reasoning_effort, redo=redo)
    return _run(options, show_progress)


def _run(options, show_progress):
    out_dir = (OUT_ROOT / options.out).resolve()
    raw_dir = out_dir / "raw"
    ...
    print(f"{utc_now()}  running batches {options.batch_start}-{options.batch_end}: "
          f"{total} papers, {options.workers} at a time, model {options.model}")

    clients = POOL
    print(f"{utc_now()}  {len(clients)} key(s), {options.workers} paper(s) at a "
          f"time, {WORKERS_PER_KEY} request(s) per key", flush=True)
    ...
    def run_one(row):
        ...
        if destination.exists() and not options.redo:
            ...
            return
        try:
            record = extract_paper(clients, row, options, step, step_done)
            ...
            try:
                ...
                if WRITE_EVERY and (state["ok"] + 1) % WRITE_EVERY == 0:
                    with write_lock:
...
# =================================== run =====================================
written = run(
    batch_start=BATCH_START, batch_end=BATCH_END, batches=BATCHES, papers=PAPERS,
    out=OUT, model=MODEL, base_url=BASE_URL,
    workers=WORKERS, attempts=ATTEMPTS, max_tokens=MAX_TOKENS,
    reasoning_effort=REASONING_EFFORT, redo=REDO,
)
print(written)
...
TABLES = ("extraction_status", "study_contexts", "observations",
          "entity_mentions", "evidence_spans", "paper_profiles",
          "training_pairs", "african_innovation")
...
if loaded.get("extraction_status") is not None and len(loaded["extraction_status"]):
    status = loaded["extraction_status"]
    print(f"\npapers finished: {len(status)}")
    print(f"mean seconds per paper: {status['seconds'].mean():.0f}")
    print(f"completion tokens: {status['completion_tokens'].sum():,.0f}")

if loaded.get("training_pairs") is not None and len(loaded["training_pairs"]):
    pairs = loaded["training_pairs"]
    print("\ntraining pairs by type:", dict(pairs.pair_type.value_counts()))
    kinds = pairs[pairs.pair_kind != ""].pair_kind.value_counts()
    print("reasoning kinds:", dict(kinds))

if loaded.get("african_innovation") is not None and len(loaded["african_innovation"]):
    innovation = loaded["african_innovation"]
    print("\nafrican innovation by type:", dict(innovation.innovation_type.value_counts()))
    display(innovation[["paper_id", "innovation_type", "constraint_addressed",
                        "place", "what_is_distinctive"]].head(10))

if loaded.get("observations") is not None and len(loaded["observations"]):
    print("observations by kind:", dict(loaded["observations"].statement_kind.value_counts()))
